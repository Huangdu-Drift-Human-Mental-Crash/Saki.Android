...
public class AlacDecodeUtils
{
	...
	static int readbits_16(AlacFile alac, int bits )
	{
		...
		part1 = inputByteAt(alac, alac.ibIdx);
		part2 = inputByteAt(alac, alac.ibIdx + 1);
		part3 = inputByteAt(alac, alac.ibIdx + 2);
	...
	{
		int bitPosition = (alac.ibIdx * 8) + alac.input_buffer_bitaccumulator - bits;
		if (bits < 0 || bitPosition < 0)
			throw new IllegalArgumentException("Cannot unread before the ALAC packet start");
		alac.ibIdx = bitPosition / 8;
		alac.input_buffer_bitaccumulator = bitPosition % 8;
	}

	private static int inputByteAt(AlacFile alac, int index)
	{
		return index >= 0 && index < alac.input_buffer_limit
				? (alac.input_buffer[index] & 0xff)
				: 0;
	}

	private static void ensureBitsAvailable(AlacFile alac, int bits)
	{
		if (bits < 0 || bits > 16)
			throw new IllegalArgumentException("Invalid ALAC bit read size: " + bits);
		long bitPosition = ((long) alac.ibIdx * 8) + alac.input_buffer_bitaccumulator;
		long remainingBits = ((long) alac.input_buffer_limit * 8) - bitPosition;
		if (bits > remainingBits)
			throw new IllegalArgumentException("Truncated ALAC packet");
	}
	...
	public static int decode_frame(AlacFile alac, byte[] inbuffer, int inputLength, int[] outbuffer, int outputsize )
	{
		if (inputLength < 0 || inputLength > inbuffer.length)
			throw new IllegalArgumentException("Invalid ALAC packet length: " + inputLength);
		int channels ;
		int outputsamples  = alac.setinfo_max_samples_per_frame;

		/* setup the stream */
		alac.input_buffer = inbuffer;
		alac.input_buffer_limit = inputLength;
		...
		outputsize = validateOutputSize(alac, outputsamples, outputsize);
		...
		if(channels == 0) // 1 channel
		{
			...
			{ // so it is compressed
				...
				{
					throw new IllegalArgumentException("Unsupported ALAC prediction type: " + prediction_type);
				}
			...
			case 32:
				throw new IllegalArgumentException(
						"Unsupported ALAC sample size: " + alac.setinfo_sample_size);
			default:
		...
		{
			if (alac.numchannels != 2)
				throw new IllegalArgumentException("Stereo ALAC packet in a non-stereo stream");
			int hassize ;
			int isnotcompressed ;
			int readsamplesize ;
			...
			{ // compressed
				...
				{ // adaptive fir
					...
					throw new IllegalArgumentException("Unsupported ALAC prediction type: " + prediction_type_a);
				...
				{ // adaptive fir
					...
					throw new IllegalArgumentException("Unsupported ALAC prediction type: " + prediction_type_b);
			}
			else
			{ // not compressed, easy case
				if (alac.setinfo_sample_size <= 16)
		...
		{
			throw new IllegalArgumentException("Unsupported ALAC channel element: " + channels);
		}
		return outputsize;
	}

	private static int validateOutputSize(AlacFile alac, int outputSamples, int maximumOutputSize)
	{
		if (outputSamples <= 0 || outputSamples > alac.setinfo_max_samples_per_frame ||
				outputSamples > alac.outputsamples_buffer_a.length)
			throw new IllegalArgumentException("Invalid ALAC frame sample count: " + outputSamples);
		long decodedSize = (long) outputSamples * alac.bytespersample;
		if (decodedSize > maximumOutputSize)
			throw new IllegalArgumentException("ALAC frame exceeds the output buffer");
